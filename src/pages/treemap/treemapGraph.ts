...
import {createTooltip, toggleTooltip, switchOffTooltip, doListEffect} from "../../util/tooltip";
...
export const showTreemap = (selector: string, data: Tree, width: number, height: number, newZoomtoCallback: (newZoomto: string) => void, zoomto: string) => {
    ...
    function render(group, _root, _zoomto?: string) {
        ...
        node.append("rect")
            ...
            .attr("fill", lowlight)
            .attr("stroke", "#fff")
            .on("contextmenu", (event: MouseEvent, d) => toggleTooltip(tooltip, event, d.leafUid, d))
